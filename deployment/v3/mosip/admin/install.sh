...
function installing_admin() {
  ...
  helm -n $NS install admin-service mosip/admin-service -f values.yaml --set istio.corsPolicy.allowOrigins\[0\].prefix=https://$ADMIN_HOST --wait --version $CHART_VERSION
